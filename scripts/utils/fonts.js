...
// Kept close to the upstream minified form on purpose, so disable the lint
// rules that don't apply to this vendor snippet.
function dynamicTypekit(kitId, d = document) {
  const config = { kitId, scriptTimeout: 3000, async: true };
  /* eslint-disable no-multi-assign, func-names, no-mixed-operators, eqeqeq, no-undef, no-empty */
  ...
  /* eslint-enable no-multi-assign, func-names, no-mixed-operators, eqeqeq, no-undef, no-empty */
  return h;
}
