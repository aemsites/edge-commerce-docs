...
| `jsonld` | string | No | Custom schema.org JSON-LD override. Replaces generated JSON-LD entirely. max length 128000 |
| `custom` | Record<string, any> | No | Custom data bag preserved in responses. |
| `jsonldExtensions` | Record<string, any> | No | Additional schema.org properties shallow-merged into generated Product JSON-LD. max length 32000 |
| `shipping` | string \| object \| object[] | No | Shipping information for merchant feed output. |
| `bundleItems` | object[] | No | Bundle composition entries for bundle products. |
| `feeds` | object | No | Feed configuration for product distribution. |
| `weight` | [ProductBusWeight](#productbusweight) | No | Product weight for JSON-LD structured data. |
| `shippingDimensions` | [ShippingDimensions](#shippingdimensions) | No | Product shipping dimensions emitted as Offer.shippingDetails in JSON-LD. |
| `taxCode` | string | No | Tax classification code. max length 255 |
| `taxData` | Record<string, any> | No | Additional tax provider data. |
...
| `availability` | [SchemaOrgAvailability](#schemaorgavailability) | No | Stock status using schema.org availability vocabulary. |
| `options` | object[] | No | Selected option values that identify this variant. |
...
| `jsonldExtensions` | Record<string, any> | No | Additional schema.org properties shallow-merged into this variant's Offer in generated JSON-LD. max length 16000 |
| `weight` | [ProductBusWeight](#productbusweight) | No | Variant weight for JSON-LD structured data. |
...
| Field | Type | Required | Description |
|---|---|---|---|
| `final` | string | No | Final price the customer pays, as a decimal string. |
...
| `dimensionsUnit` | `cm` \| `mm` \| `in` | No | Unit for height, width, and depth. Maps to UN/CEFACT codes in JSON-LD output. |
...
| `video` | string | No | Associated video URL. |
...
| `reviewCount` | string | No | Number of reviews, e.g. "127". Converts to an integer in JSON-LD. |
