...
reCAPTCHA Enterprise protects the API's unauthenticated write endpoints from automated abuse. When configured, the API verifies a reCAPTCHA token on each protected request (order creation, order preview, and login) and rejects requests that fail the assessment. This is the API's first line of defense against bots and card-testing attacks before payment is ever initiated.

Unlike the payment, fraud, tax, and identity providers, reCAPTCHA is not tied to a single checkout step and is configured as a **global** secret only; it does not use country or locale scoping.
...
Because reCAPTCHA is global-only, write it to the root path with no country or locale prefix:
...
| `apiKey` | string | **Yes** | A Google Cloud API key restricted to the reCAPTCHA Enterprise API, used server-side to request assessments. Treat as a secret; never log or expose. Typically 30–80 characters |

Of the three, only `apiKey` is a true secret. `projectId` and `siteKey` are public identifiers that are stored server-side so the API can validate tokens against the correct project and key.
